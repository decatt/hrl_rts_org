...
import torch
import numpy as np
from hrl_code.tools.mask import CategoricalMasked
...
class WorkerAgent:
    ...
    def get_action(self, x, g, unit_masks=None, action_masks=None):
        ...
        a_masks = torch.zeros((len(action_components), 78))
        for i in range(len(action_components)):
            a_masks[i] = action_masks[i][action_components[i]]
        ...
        log_prob = torch.stack([categorical.log_prob(a) for a, categorical in zip(action, multi_categoricals)])
        return action.T.cpu().numpy(), log_prob.cpu().sum(0).numpy().reshape(-1), masks.cpu().numpy(), value.cpu().numpy().reshape(-1)
...
class ManagerNet(nn.Module):
    def __init__(self, goal_length=12):
        ...
        self.actor_m = layer_init(nn.Linear(256, goal_length), std=0.01)
...
class ManagerAgent:
    ...
    def get_goal(self, x, std=0.01):
        ...
        return goals, value, log_probs.sum(1)
    ...
    def get_entropy(self, x, a, std=0.01):
        ...
        normal_distributions = torch.distributions.Normal(m, std)
        log_probs = normal_distributions.log_prob(a).reshape(self.num_env, 12)
        entropy = normal_distributions.entropy().reshape(self.num_env, 12)
