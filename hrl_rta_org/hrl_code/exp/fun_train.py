...
from hrl_code.agents.fun_agent import WorkerAgent, WorkerNet, ManagerAgent, ManagerNet
...
if __name__ == '__main__':
    ...
    for games in range(100):
        for step in range(5000):
            ...
            obs = next_obs
            with torch.no_grad():
                goals, _, _ = manager_agent.get_goal(torch.Tensor(obs).to(device))
                ...
                unit_action, _, _, _ = worker_agent.get_action(torch.Tensor(obs).to(device), goals, unit_masks=unit_mask, action_masks=invalid_action_masks)
            for i in range(num_envs):
